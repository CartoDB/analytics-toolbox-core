...
CREATE OR REPLACE FUNCTION `@@BQ_PREFIX@@carto.__MINKOWSKIDISTANCE`
...
CREATE OR REPLACE FUNCTION `@@BQ_PREFIX@@carto.ST_MINKOWSKIDISTANCE`
(geog ARRAY<GEOGRAPHY>, p FLOAT64)
RETURNS ARRAY<STRING>
AS ((
    SELECT `@@BQ_PREFIX@@carto.__MINKOWSKIDISTANCE`(ARRAY_AGG(ST_ASGEOJSON(x)), p)
    FROM unnest(geog) x
));