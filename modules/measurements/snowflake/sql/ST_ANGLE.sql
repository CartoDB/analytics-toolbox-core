...
RETURNS DOUBLE
AS $$
    @@SF_PREFIX@@measurements._ANGLE(CAST(ST_ASGEOJSON(STARTPOINT) AS STRING), CAST(ST_ASGEOJSON(MIDPOINT) AS STRING), CAST(ST_ASGEOJSON(ENDPOINT) AS STRING), MERCATOR)
$$;
