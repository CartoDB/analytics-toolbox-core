...
test('ST_CENTEROFMASS should work', async () => {
    ...
    \`@@BQ_PREFIX@@carto.ST_CENTEROFMASS\`(ST_GEOGFROMTEXT("POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))")) as centerOfMass2,
    \`@@BQ_PREFIX@@carto.ST_CENTEROFMASS\`(ST_GEOGFROMTEXT("POLYGON ((-120 30, -90 40, 20 40, -45 -20, -120 30))")) as centerOfMass3`;
...
test('ST_CENTEROFMASS should return NULL if any NULL mandatory argument', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@carto.ST_CENTEROFMASS`(NULL) as centerofmass1';
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
