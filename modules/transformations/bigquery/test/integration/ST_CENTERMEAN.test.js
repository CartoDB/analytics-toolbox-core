...
test('ST_CENTERMEAN should work', async () => {
    ...
    \`@@BQ_PREFIX@@carto.ST_CENTERMEAN\`(ST_GEOGFROMTEXT("POLYGON ((30 10, 40 40, 20 40, 10 20, 30 10))")) as centerMean2,
    \`@@BQ_PREFIX@@carto.ST_CENTERMEAN\`(ST_GEOGFROMTEXT("POLYGON ((-120 30, -90 40, 20 40, -45 -20, -120 30))")) as centerMean3`;
...
test('ST_CENTERMEAN should return NULL if any NULL mandatory argument', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@carto.ST_CENTERMEAN`(NULL) as centermean1';
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
