...
CREATE OR REPLACE FUNCTION _DESTINATION
...
RETURNS GEOGRAPHY
AS $$
    TO_GEOGRAPHY(_DESTINATION(CAST(ST_ASGEOJSON(STARTPOINT) AS STRING), DISTANCE, BEARING, 'kilometers'))
$$;
...
RETURNS GEOGRAPHY
AS $$
    TO_GEOGRAPHY(_DESTINATION(CAST(ST_ASGEOJSON(STARTPOINT) AS STRING), DISTANCE, BEARING, UNITS))
$$;