...
transformations.ST_CONCAVEHULL(geojsons)
{{%/ bannerNote %}}
...
{{% bannerNote type="code" %}}
transformations.ST_CONCAVEHULL(geojsons, maxEdge)
{{%/ bannerNote %}}
...
* `geojsons`: `ARRAY` array of features in GeoJSON format casted to STRING.
* `maxEdge`: `DOUBLE`|`NULL` the length (in 'units') of an edge necessary for part of the hull to become concave. If `NULL`the default value `infinity` is used.
