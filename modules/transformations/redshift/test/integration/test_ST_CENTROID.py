...
def test_center_mean_success():
    ...
    results = run_query(
        f"""SELECT ST_ASTEXT(@@RS_PREFIX@@transformations.ST_CENTROID(
            ST_GeomFromText('{points[0].rstrip()}'))),
        ST_ASTEXT(@@RS_PREFIX@@transformations.ST_CENTROID(
    ...
    assert str(results[0][0]) == 'POINT(4.840755 45.756063)'
    assert str(results[0][1]) == 'POINT(25.454545 26.969697)'
    assert str(results[0][2]) == 'POINT(50.19774 -19.152542)'
    assert str(results[0][3]) == 'POINT(-3.790576 37.77855)'


def test_center_mean_none():
    results = run_query(
        """SELECT @@RS_PREFIX@@transformations.ST_CENTROID(
