...
test('ST_VORONOILINES should work', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@carto.ST_VORONOILINES\`([${pointsArray}], [-76.0, 35.0, -70.0, 45.0]) as voronoi`;
...
test('ST_VORONOILINES should work with null bbox', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@carto.ST_VORONOILINES\`([${pointsArray}], null) as voronoi`;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
...
test('ST_VORONOILINES should return an empty array if passed null geometry', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@carto.ST_VORONOILINES`(null, null) as voronoi';
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
...
test('ST_VORONOILINES should fail if passed invalid bbox', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@carto.ST_VORONOILINES\`([${pointsArray}], [1.0, 0.5, 2.5])`;
