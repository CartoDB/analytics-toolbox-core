...
test('ST_POLYGONIZE should work', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@processing.ST_POLYGONIZE\`([${fixturesIn.linesArray}]) as polygons`;
...
test('ST_POLYGONIZE should fail if a degenerated line path is received', async () => {
    const query =  `SELECT \`@@BQ_PREFIX@@processing.ST_POLYGONIZE\`(${fixturesIn.degeneratedLine})`;
...
test('ST_POLYGONIZE should fail if a path with less than 3 nodes is received', async () => {
    const query =  `SELECT \`@@BQ_PREFIX@@processing.ST_POLYGONIZE\`(${fixturesIn.invalidPath})`;
    await expect(runQuery(query)).rejects.toThrow(
