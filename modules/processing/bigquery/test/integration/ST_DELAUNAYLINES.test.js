...
test('ST_DELAUNAYLINES should work', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@processing.ST_DELAUNAYLINES\`(${fixturesIn.input1}) as delaunay`;
...
test('ST_DELAUNAYLINES should return an empty array if passed null geometry', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@processing.ST_DELAUNAYLINES`(null) as delaunay';
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
