...
test.skip('ST_DELAUNAYPOLYGONS should work', async () => {
    const query = `SELECT \`@@BQ_PREFIX@@carto.ST_DELAUNAYPOLYGONS\`(${fixturesIn.input2}) as delaunay`;
...
test('ST_DELAUNAYPOLYGONS should return an empty array if passed null geometry', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@carto.ST_DELAUNAYPOLYGONS`(null) as delaunay';
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
