...
test('TOPARENT should work at any level of zoom', async () => {
    const query = `WITH zoomContext AS(
        ...
        (
            SELECT *,
            ST_ASQUADINT(ST_POINT(long, lat), zoom - 1) AS expectedParent,
            TOPARENT(
                ST_ASQUADINT(ST_POINT(long, lat), zoom),zoom - 1) AS parent
            FROM zoomContext
        )
...
test('TOPARENT should reject quadints at zoom 0', async () => {
    const query = 'SELECT TOPARENT(0,0)';
    await expect(runQuery(query)).rejects.toThrow();
});

test('TOPARENT should fail with NULL argument', async () => {
    let query = 'SELECT TOPARENT(NULL, 10);';
    await expect(runQuery(query)).rejects.toThrow();

    query = 'SELECT TOPARENT(322, NULL);';
    await expect(runQuery(query)).rejects.toThrow();
});