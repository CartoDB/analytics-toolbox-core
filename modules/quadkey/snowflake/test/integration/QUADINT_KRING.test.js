...
test('QUADINT_KRING should work', async () => {
    const query = `
        SELECT QUADINT_KRING(GET(VALUE,'origin'), GET(VALUE,'size')) as kring
...
test('QUADINT_KRING should fail if any invalid argument', async () => {
    let query = 'SELECT QUADINT_KRING(NULL, NULL)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT QUADINT_KRING(-1, 1)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT QUADINT_KRING(162, -1)';
