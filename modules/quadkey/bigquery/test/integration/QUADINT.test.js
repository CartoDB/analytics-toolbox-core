...
test('QUADKEY should be able to encode/decode between quadint and quadkey at any level of zoom', async () => {
    const query = `WITH tileContext AS (
        ...
        FROM (
            SELECT *,
                \`@@BQ_PREFIX@@carto.QUADINT_TOZXY\`(
                    \`@@BQ_PREFIX@@carto.QUADINT_FROMQUADKEY\`(
                        \`@@BQ_PREFIX@@carto.QUADINT_TOQUADKEY\`(
                            \`@@BQ_PREFIX@@carto.QUADINT_FROMZXY\`(zoom, tileX, tileY)))) AS decodedQuadkey
            FROM tileContext
        )
...
test('QUADINT_TOQUADKEY should fail with NULL argument', async () => {
    let query = 'SELECT `@@BQ_PREFIX@@carto.QUADINT_TOQUADKEY`(NULL);';
