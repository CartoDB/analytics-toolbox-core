...
test('LONGLAT_ASQUADINTLIST_RESOLUTION should work', async () => {
    const query = `
        SELECT
            \`@@BQ_PREFIX@@quadkey.LONGLAT_ASQUADINTLIST_RESOLUTION\`(-45,30,2,10,2,5) AS agg1,
            \`@@BQ_PREFIX@@quadkey.LONGLAT_ASQUADINTLIST_RESOLUTION\`(150,-30,16,18,1,5) AS agg2,
            \`@@BQ_PREFIX@@quadkey.LONGLAT_ASQUADINTLIST_RESOLUTION\`(170,-80,20,24,1,4) AS agg3`;
...
test('LONGLAT_ASQUADINTLIST_RESOLUTION should return [] with NULL latitud/longitud', async () => {
    const query = `
        SELECT
            \`@@BQ_PREFIX@@quadkey.LONGLAT_ASQUADINTLIST_RESOLUTION\`(NULL,10,10,12,1,5) AS agg1,
            \`@@BQ_PREFIX@@quadkey.LONGLAT_ASQUADINTLIST_RESOLUTION\`(10,NULL,10,12,1,5) AS agg2`;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(rows[0].agg1).toEqual([]);
    expect(rows[0].agg2).toEqual([]);
