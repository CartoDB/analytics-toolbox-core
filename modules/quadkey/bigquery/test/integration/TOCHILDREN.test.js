...
test('TOCHILDREN should work at any level of zoom', async () => {
    const query = `WITH zoomContext AS (
        ...
        childrenContext AS (
            SELECT *,
            \`@@BQ_PREFIX@@quadkey.TOCHILDREN\`(expectedQuadint, zoom + 1) AS children
            FROM expectedQuadintContext 
        )
        SELECT *
        FROM (
            SELECT expectedQuadint,
            \`@@BQ_PREFIX@@quadkey.TOPARENT\`(child, zoom) AS currentQuadint
...
test('TOCHILDREN should reject quadints at zoom 29', async () => {
    const query = 'SELECT `@@BQ_PREFIX@@quadkey.TOCHILDREN`(4611686027017322525,30)';
    await expect(runQuery(query)).rejects.toThrow();
});

test('TOCHILDREN should fail with NULL arguments', async () => {
    let query = 'SELECT `@@BQ_PREFIX@@quadkey.TOCHILDREN`(NULL, 1);';
    await expect(runQuery(query)).rejects.toThrow();

    query = 'SELECT `@@BQ_PREFIX@@quadkey.TOCHILDREN`(322, NULL);';
    await expect(runQuery(query)).rejects.toThrow();
});