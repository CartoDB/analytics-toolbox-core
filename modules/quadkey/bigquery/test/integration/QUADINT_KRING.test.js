...
test('QUADINT_KRING should work', async () => {
    const query = `
        WITH kring_data AS
        ( SELECT row,
            \`@@BQ_PREFIX@@quadkey.QUADINT_KRING\`(origin, size) as kring_elem,
        FROM UNNEST([
...
test('QUADINT_KRING should fail if any invalid argument', async () => {
    let query = 'SELECT `@@BQ_PREFIX@@quadkey.QUADINT_KRING`(NULL, NULL);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_PREFIX@@quadkey.QUADINT_KRING`(-1, 1);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_PREFIX@@quadkey.QUADINT_KRING`(162, -1);';
