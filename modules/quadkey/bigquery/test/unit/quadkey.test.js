...
test('bbox should work', () => {
    ...
    expect(lib.bbox(12960460429066265n)).toEqual([-45, 44.999994612636684, -44.99998927116394, 45.00000219906962]);
});
    ...
    for (z = 1; z < 30; ++z) {
        for (lat = -90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                const currentParent = lib.quadintFromLocation(lng, lat, z - 1);
                expect(currentParent).toEqual(lib.toParent(quadint, z - 1));
            }
        }
    }
    for (z = 5; z < 30; ++z) {
        for (lat = -90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                const currentParent = lib.quadintFromLocation(lng, lat, z - 5);
                expect(currentParent).toEqual(lib.toParent(quadint, z - 5));
            }
        }
    }
    for (z = 10; z < 30; ++z) {
        for (lat = -90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                const currentParent = lib.quadintFromLocation(lng, lat, z - 10);
                expect(currentParent).toEqual(lib.toParent(quadint, z - 10));
            }
        }
    }
});
...
test('toChildren should work at any level of zoom', () => {
    ...
    for (z = 0; z < 29; ++z) {
        for (lat = 90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                const childs = lib.toChildren(quadint, z + 1);
                childs.forEach((element) => {
                    expect(lib.toParent(element, z)).toEqual(quadint);
                });
            }
        }
    }
    ...
    for (z = 0; z < 25; ++z) {
        for (lat = 90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                const childs = lib.toChildren(quadint, z + 5);
                childs.forEach((element) => {
                    expect(lib.toParent(element, z)).toEqual(quadint);
                });
            }
...
test('Sibling should work at any level of zoom', async () => {
    ...
    for (z = 0; z < 29; ++z) {
        for (lat = -90; lat <= 90; lat = lat + 15) {
            for (lng = -180; lng <= 180; lng = lng + 15) {
                const quadint = lib.quadintFromLocation(lng, lat, z);
                let siblingQuadint = lib.sibling(quadint, 'right');
                siblingQuadint = lib.sibling(siblingQuadint, 'up');
                siblingQuadint = lib.sibling(siblingQuadint, 'left');
                siblingQuadint = lib.sibling(siblingQuadint, 'down');
...
test('kRing should work', async () => {
    ...
    expect(lib.kRing(12070922, 2).sort().map(String)).toEqual([
    ...
    expect(lib.kRing(791040491538, 3).sort().map(String)).toEqual([
...
test('Should be able to encode/decode between quadint and quadkey at any level of zoom', async () => {
    ...
    for (let z = 0; z < 30; ++z) {
        ...
        x = 0;
        y = 0;
        let zxyDecoded = lib.ZXYFromQuadint(lib.quadintFromQuadkey(lib.quadkeyFromQuadint(lib.quadintFromZXY(z, x, y))));
        ...
        if (z > 0) {
            x = tilesPerLevel / 2;
            y = tilesPerLevel / 2;
            zxyDecoded = lib.ZXYFromQuadint(lib.quadintFromQuadkey(lib.quadkeyFromQuadint(lib.quadintFromZXY(z, x, y))));
            zDecoded = zxyDecoded.z;
            xDecoded = zxyDecoded.x;
            yDecoded = zxyDecoded.y;
            expect(z === zDecoded && x === xDecoded && y === yDecoded).toBeTruthy();
...
test('Should be able to encode/decode tiles at any level of zoom', async () => {
    ...
    for (let z = 0; z < 30; ++z) {
        ...
        x = 0;
        y = 0;
        let zxyDecoded = lib.ZXYFromQuadint(lib.quadintFromZXY(z, x, y));
        zDecoded = zxyDecoded.z;
        xDecoded = zxyDecoded.x;
        yDecoded = zxyDecoded.y;
        expect(z === zDecoded && x === xDecoded && y === yDecoded).toBeTruthy();
        ...
        if (z > 0) {
            x = tilesPerLevel / 2;
            y = tilesPerLevel / 2;
            zxyDecoded = lib.ZXYFromQuadint(lib.quadintFromZXY(z, x, y));
            zDecoded = zxyDecoded.z;
            xDecoded = zxyDecoded.x;
            yDecoded = zxyDecoded.y;
            expect(z === zDecoded && x === xDecoded && y === yDecoded).toBeTruthy();
