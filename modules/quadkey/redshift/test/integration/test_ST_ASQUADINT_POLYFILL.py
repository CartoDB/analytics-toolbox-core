...
def test_polyfill_success():
    ...
    result = run_query(
        f"""SELECT @@RS_PREFIX@@quadkey.ST_ASQUADINT_POLYFILL(
            ST_GeomFromText('{feature_wkt}'), 10) UNION ALL
        SELECT @@RS_PREFIX@@quadkey.ST_ASQUADINT_POLYFILL(
...
def test_polyfill_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@quadkey.ST_ASQUADINT_POLYFILL(NULL, 10)')
