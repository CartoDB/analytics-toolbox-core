...
def test_toparent_success():
    results = run_query(
        ...
        SELECT @@RS_PREFIX@@carto.S2_TOPARENT(id, resolution) AS parent_id
        FROM context;"""
    )
...
def test_toparent_invalid_resolution_failure():
    ...
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.S2_TOPARENT(-7843177080740118528, 9)')
    assert 'InvalidResolution' in str(excinfo.value)


def test_toparent_null_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.S2_TOPARENT(NULL, 10)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.S2_TOPARENT(-7843177080740118528, NULL)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.S2_TOPARENT(NULL)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
