...
def test_st_asid_polyfill_bbox_success():
    results = run_query(
        ...
        SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(
...
def test_st_asid_polyfill_bbox_invalid_resolution_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-3.7,-3.68,40.4,40.42,-1,2)'
        )
    assert 'InvalidResolution' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-3.7,-3.68,40.4,40.42,0,31)'
        )
    assert 'InvalidResolution' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-3.7,-3.68,40.4,40.42,8,3)'
        )
    assert 'InvalidResolution' in str(excinfo.value)
...
def test_st_asid_polyfill_bbox_null_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(NULL, -3, 40.4, 40.5, -1, 2)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-4, NULL, 40.4, 40.5, -1, 2)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-4, -3, NULL, 40.5, -1, 2)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-4, -3, 40, NULL, -1, 2)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-4, -3, 40.4, 40.5, NULL, 2)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query(
            'SELECT @@RS_PREFIX@@s2.S2_POLYFILL_BBOX(-4, -3, 40.4, 40.5, -1, NULL)'
        )
    assert 'NULL argument passed to UDF' in str(excinfo.value)
