...
def test_longlat_asid_success():
    results = run_query(
        ...
        SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(long, lat, res) as ids
            FROM resContext;"""
    )
...
def test_longlat_asid_invalid_resolution_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(100, 100, 32)')
    assert 'InvalidResolution' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(100, 100, -1)')
    assert 'InvalidResolution' in str(excinfo.value)


def test_longlat_asid_null_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(NULL, 10, 10)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(10, NULL, 10)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.S2_IDFROMLONGLAT(10, 10, NULL)')
    assert 'NULL argument passed to UDF' in str(excinfo.value)
