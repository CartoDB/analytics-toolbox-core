...
def test_longlat_asid_success():
    results = run_query(
        ...
        SELECT @@RS_PREFIX@@s2.ST_ASID(point, res) as ids
            FROM resContext;"""
    )
...
def test_longlat_asid_invalid_resolution_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.ST_ASID(ST_POINT(100, 100), 32)')
    assert 'InvalidResolution' in str(excinfo.value)
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@s2.ST_ASID(ST_POINT(100, 100), -1)')
    assert 'InvalidResolution' in str(excinfo.value)
