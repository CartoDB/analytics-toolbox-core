...
test('Works as expected with invalid data', async () => {
    const query = `
        ...
        SELECT
            id,
            \`@@BQ_PREFIX@@h3.KRING_INDEXED\`(hid, distance) as parent
        FROM ids
        ORDER BY id ASC
    `;
...
test('List the ring correctly', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@h3.KRING_INDEXED\`(hid, 1) as d1,
            \`@@BQ_PREFIX@@h3.KRING_INDEXED\`(hid, 2) as d2
        FROM ids
    `;
...
test('Zero distance returns self', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@h3.KRING_INDEXED\`(hid, 0) AS self_children
        FROM ids
    `;
