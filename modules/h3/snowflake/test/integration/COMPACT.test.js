...
test('Work as expected with NULLish values', async () => {
    let query = `
        SELECT 
        @@SF_PREFIX@@h3.COMPACT(NULL) as c,
        @@SF_PREFIX@@h3.UNCOMPACT(NULL, 5) as u
    `;
    ...
    query = `
        SELECT 
        @@SF_PREFIX@@h3.COMPACT(ARRAY_CONSTRUCT()) as c,
        @@SF_PREFIX@@h3.UNCOMPACT(ARRAY_CONSTRUCT(), 5) as u
    `;
