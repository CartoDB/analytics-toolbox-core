...
test('H3_HEXRING should work', async () => {
    const query = `
        SELECT H3_HEXRING('8928308280fffff', 0) as d0,
               H3_HEXRING('8928308280fffff', 1) as d1,
               H3_HEXRING('8928308280fffff', 2) as d2
    `;
...
test('H3_HEXRING should fail if any invalid argument', async () => {
    ...
    query = 'SELECT H3_HEXRING(\'8928308280fffff\', -1)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input size/);
});