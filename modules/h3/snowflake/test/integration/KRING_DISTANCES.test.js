...
test('Works as expected with invalid data', async () => {
    const query = `
        ...
        SELECT
            id,
            @@SF_PREFIX@@h3.KRING_DISTANCES(origin, size) as parent
        FROM ids
        ORDER BY id ASC
    `;
...
test('List the ring correctly', async () => {
    const query = `
        ...
        SELECT
            @@SF_PREFIX@@h3.KRING_DISTANCES(origin, 1) as d1,
            @@SF_PREFIX@@h3.KRING_DISTANCES(origin, 2) as d2
        FROM ids
    `;
...
test('Zero size returns self', async () => {
    const query = `
        ...
        SELECT
            @@SF_PREFIX@@h3.KRING_DISTANCES(origin, 0) AS self_children
        FROM ids
    `;
