...
test('Works as expected with invalid data', async () => {
    const query = `
        ...
        SELECT
            id,
            \`@@BQ_PREFIX@@h3.HEXRING\`(origin, size) as parent
        FROM ids
        ORDER BY id ASC
    `;
...
test('List the ring correctly', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@h3.HEXRING\`(origin, 1) as d1,
            \`@@BQ_PREFIX@@h3.HEXRING\`(origin, 2) as d2
        FROM ids
    `;
...
test('Zero size returns self', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@h3.HEXRING\`(origin, 0) AS self_children
        FROM ids
    `;
