...
test('H3_TOCHILDREN works as expected with invalid data', async () => {
    const query = `
        ...
        SELECT
            id,
            \`@@BQ_PREFIX@@carto.H3_TOCHILDREN\`(hid, 1) as parent
        FROM ids
        ORDER BY id ASC
    `;
...
test('List children correctly', async () => {
    const query = `
        ...
        (
            SELECT
                \`@@BQ_PREFIX@@carto.H3_FROMGEOGPOINT\`(ST_GEOGPOINT(-122.409290778685, 37.81331899988944), 7) AS hid
        )
        SELECT
            ARRAY_LENGTH(\`@@BQ_PREFIX@@carto.H3_TOCHILDREN\`(hid, 8)) AS length_children,
            ARRAY_LENGTH(\`@@BQ_PREFIX@@carto.H3_TOCHILDREN\`(hid, 9)) AS length_grandchildren
        FROM ids
    `;
...
test('Same resolution lists self', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@carto.H3_TOCHILDREN\`(hid, 7) AS self_children
        FROM ids
    `;
...
test('Coarser resolution returns empty array', async () => {
    const query = `
        ...
        SELECT
            \`@@BQ_PREFIX@@carto.H3_TOCHILDREN\`(hid, 6) AS top_children
        FROM ids
    `;
