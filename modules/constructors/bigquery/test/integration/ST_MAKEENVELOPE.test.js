...
test('ST_MAKEENVELOPE should work', async () => {
    const query = `
        SELECT \`@@BQ_PREFIX@@constructors.ST_MAKEENVELOPE\`(10.0, 10.0, 11.0, 11.0) as poly
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(rows[0].poly.value).toEqual('POLYGON((11 10, 11 11, 10 11, 10 10, 11 10))');
});
...
test('ST_MAKEENVELOPE should return NULL if any NULL argument', async () => {
    const query = `
        SELECT \`@@BQ_PREFIX@@constructors.ST_MAKEENVELOPE\`(-179.0, 10.0, 179.0, 11.0) as poly1,
               \`@@BQ_PREFIX@@constructors.ST_MAKEENVELOPE\`(179.0, 10.0, -179.0, 11.0) as poly2
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(rows[0].poly1.value).toEqual('POLYGON((-179 10, -179 11, 179 11, 179 10, -179 10))');
