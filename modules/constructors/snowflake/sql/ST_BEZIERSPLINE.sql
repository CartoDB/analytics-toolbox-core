...
CREATE OR REPLACE FUNCTION __BEZIERSPLINE
...
IMMUTABLE
AS $$
    TO_GEOGRAPHY(__BEZIERSPLINE(CAST(ST_ASGEOJSON(GEOG) AS STRING), 10000, 0.85))
$$;
...
IMMUTABLE
AS $$
    TO_GEOGRAPHY(__BEZIERSPLINE(CAST(ST_ASGEOJSON(GEOG) AS STRING), CAST(RESOLUTION AS DOUBLE), 0.85))
$$;
...
IMMUTABLE
AS $$
    TO_GEOGRAPHY(__BEZIERSPLINE(CAST(ST_ASGEOJSON(GEOG) AS STRING), CAST(RESOLUTION AS DOUBLE), SHARPNESS))
$$;
