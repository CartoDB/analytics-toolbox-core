...
test('ST_BEZIERSPLINE should work', async () => {
    ...
    expect(JSON.stringify(rows[0].BEZIERSPLINE2)).toEqual('{"coordinates":[[-6,-0.5],[-3.6649305555555554,0.4259259259259258],[-1.0611111111111116,-0.24074074074074053],[1.5,0],[4.06111111111111,0.24074074074074117],[6.664930555555557,-0.42592592592592565]],"type":"LineString"}');
});
...
test('ST_BEZIERSPLINE default values should work', async () => {
    const query = `
        SELECT @@SF_PREFIX@@constructors.ST_BEZIERSPLINE(ST_GEOGFROMTEXT('LINESTRING (-76.091308 18.427501,-76.695556 18.729501,-76.552734 19.40443,-74.61914 19.134789,-73.652343 20.07657,-73.157958 20.210656)'), 10000, 0.85) as defaultValue,
               @@SF_PREFIX@@constructors.ST_BEZIERSPLINE(ST_GEOGFROMTEXT('LINESTRING (-76.091308 18.427501,-76.695556 18.729501,-76.552734 19.40443,-74.61914 19.134789,-73.652343 20.07657,-73.157958 20.210656)'), NULL, NULL) as nullParam
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
