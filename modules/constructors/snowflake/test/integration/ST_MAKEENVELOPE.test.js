...
test('ST_MAKEENVELOPE should work', async () => {
    const query = `
        SELECT @@SF_PREFIX@@constructors.ST_MAKEENVELOPE(10.0, 10.0, 11.0, 11.0) as poly
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(JSON.stringify(rows[0].POLY)).toEqual('{"coordinates":[[[10,10],[10,11],[11,11],[11,10],[10,10]]],"type":"Polygon"}');
});
...
test('ST_MAKEENVELOPE should return NULL if any NULL argument', async () => {
    const query = `
        SELECT @@SF_PREFIX@@constructors.ST_MAKEENVELOPE(-179.0, 10.0, 179.0, 11.0) as poly1,
               @@SF_PREFIX@@constructors.ST_MAKEENVELOPE(179.0, 10.0, -179.0, 11.0) as poly2
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(JSON.stringify(rows[0].POLY1)).toEqual('{"coordinates":[[[-179,10],[-179,11],[179,11],[179,10],[-179,10]]],"type":"Polygon"}');
