...
def test_polyfill_success():
    fixture_file = open(
        './test/integration/st_asquadbin_polyfill_fixtures/in/wkts.txt', 'r'
    )
    ...
    result = run_query(
        ...
        SELECT @@RS_PREFIX@@carto.QUADBIN_POLYFILL(
            ST_GeomFromText('{feature_wkt}'), 14)"""
    )

    fixture_file = open(
        './test/integration/st_asquadbin_polyfill_fixtures/out/quadbins.txt', 'r'
    )
    lines = fixture_file.readlines()
    fixture_file.close()
...
def test_polyfill_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.QUADBIN_POLYFILL(NULL, 10)')
