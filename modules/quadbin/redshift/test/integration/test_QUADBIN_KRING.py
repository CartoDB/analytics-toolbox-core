...
def test_kring_success():
    result = run_query(
        """SELECT @@RS_PREFIX@@carto.QUADBIN_KRING(630503947831869440, 1),
            @@RS_PREFIX@@carto.QUADBIN_KRING(2943806043928395776, 1),
            @@RS_PREFIX@@carto.QUADBIN_KRING(2943806043928395776, 2)"""
    )
...
def test_kring_invalid_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.QUADBIN_KRING(NULL, NULL)')
    assert 'Invalid input origin' in str(excinfo.value)

    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.QUADBIN_KRING(-1, 1)')
    assert 'Invalid input origin' in str(excinfo.value)

    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.QUADBIN_KRING(630503947831869440, -1)')
