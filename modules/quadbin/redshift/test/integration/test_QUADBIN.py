...
def test_quadkey_conversion_success():
    result = run_query(
        """SELECT @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(2, 1, 1)) as quadkey1,
        @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(6, 40, 55)) as quadkey2,
        @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(12, 1960, 3612)) as quadkey3,
        @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(18, 131621, 65120)) as quadkey4,
        @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(24, 9123432, 159830174)) as quadkey5,
        @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
        @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(29, 389462872, 207468912)) as quadkey6"""
    )
...
def test_quadbin_decoding_success():
    results = run_query(
        """WITH tileContext AS(
        ...
        SELECT @@RS_PREFIX@@carto.QUADBIN_TOZXY(
            @@RS_PREFIX@@carto.QUADBIN_FROMQUADKEY(
                @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(
                    @@RS_PREFIX@@carto.__QUADBIN_FROMZXY(
                        zoom, tileX, tileY)))) AS decodedQuadkey
        FROM tileContext;"""
    )

    fixture_file = open('./test/integration/quadbin_fixtures/out/quadbins.txt', 'r')
...
def test_quadbin_null_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_PREFIX@@carto.__QUADBIN_TOQUADKEY(NULL)')
