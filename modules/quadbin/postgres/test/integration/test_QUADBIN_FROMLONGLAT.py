...
def test_quadbin_longlat():
    ...
    result = run_query(f"""SELECT QUADBIN_FROMLONGLAT(40.4168, -3.7038, 4)""")
...
def test_quadbin_longlat_null_input():
    ...
    result = run_query(f"""
      SELECT
        QUADBIN_FROMLONGLAT(NULL, -3.7038, 4) AS n1,
        QUADBIN_FROMLONGLAT(40.4168, NULL, 4) AS n2,
        QUADBIN_FROMLONGLAT(40.4168, -3.7038, NULL) AS n3
    """)
...
def test_quadbin_longlat_neg_resolution():
    ...
    with pytest.raises(Exception):
      run_query(f"""SELECT QUADBIN_FROMLONGLAT(40.4168,, -3.7038, -1)""")

def test_quadbin_longlat_large_resolution():
    """Throws error for large resolution"""
    with pytest.raises(Exception):
      run_query(f"""SELECT QUADBIN_FROMLONGLAT(40.4168,, -3.7038, 27)""")
