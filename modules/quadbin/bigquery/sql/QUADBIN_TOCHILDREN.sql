...
CREATE OR REPLACE FUNCTION `@@BQ_PREFIX@@carto.QUADBIN_TOCHILDREN`
...
AS ((
    IF(resolution < 0 OR resolution > 26 OR resolution < ((quadbin >> 52) & 0x1F),
        ...
        WITH
        __zxy AS (
            SELECT `@@BQ_PREFIX@@carto.QUADBIN_TOZXY`(quadbin) AS tile
        )
        SELECT ARRAY_AGG(`@@BQ_PREFIX@@carto.QUADBIN_FROMZXY`(resolution, xs, ys))
        FROM
            __zxy,
