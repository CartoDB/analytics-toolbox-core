...
`;
        ...
        assert.equal(rows.length, 2);
        expect(rows[0].EXPECTED).to.roughly.deep.equal(rows[0].BOUNDS);
        expect(rows[1].EXPECTED).to.roughly.deep.equal(rows[1].BOUNDS);
    });
