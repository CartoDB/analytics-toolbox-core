...
describe('H3_HEXRING', () => {
    ...
    it ('H3_KRING works as expected with invalid data', async () => {
        const query = `
WITH ids AS
(
    -- Invalid parameters
...
SELECT
    id,
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.H3_HEXRING(hid, distance) as parent
FROM ids
ORDER BY id ASC
`;
...
SELECT
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.H3_HEXRING(hid, 1) as d1,
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.H3_HEXRING(hid, 2) as d2
FROM ids
`;
...
SELECT
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.H3_HEXRING(hid, 0) AS self_children
FROM ids
`;
