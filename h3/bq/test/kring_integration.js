...
SELECT
    id,
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.KRING(hid, distance) as parent
FROM ids
ORDER BY id ASC
`;
...
SELECT
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.KRING(hid, 1) as d1,
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.KRING(hid, 2) as d2
FROM ids
`;
...
SELECT
    \`${BQ_PROJECTID}\`.\`${BQ_DATASET_H3}\`.KRING(hid, 0) AS self_children
FROM ids
`;
