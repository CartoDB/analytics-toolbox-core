...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_PROCESSING@@.__DELAUNAYGENERIC`
...
AS (
   (
        ...
        voronoi AS (
            ...
            SELECT `@@BQ_PROJECTID@@`.@@BQ_DATASET_PROCESSING@@.__VORONOIGENERIC(arrayPoints, 
                [xMin - xExtent * 0.5, 
                yMin - yExtent * 0.5, 
                xMax + xExtent * 0.5, 
                yMax + yExtent * 0.5],
        ...
        SELECT ARRAY( SELECT ST_MAKELINE([p1.dual.point, p2.dual.point, p3.dual.point, p1.dual.point]) AS triangle
                    FROM triplets AS p1
                    JOIN triplets AS p2
...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_PROCESSING@@.ST_DELAUNAYPOLYGONS`
    (points ARRAY<GEOGRAPHY>)
    RETURNS ARRAY<GEOGRAPHY>
AS (
    (
        SELECT ARRAY( SELECT ST_MAKEPOLYGON(unnested) FROM UNNEST(
                (SELECT `@@BQ_PROJECTID@@`.@@BQ_DATASET_PROCESSING@@.__DELAUNAYGENERIC(points))
            ) AS unnested
        )
    )
);

CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_PROCESSING@@.ST_DELAUNAYLINES`
    (points ARRAY<GEOGRAPHY>)
    RETURNS ARRAY<GEOGRAPHY>
AS (
    (
