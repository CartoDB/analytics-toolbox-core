...
const SF_SCHEMA_SQUELLETON = process.env.SF_SCHEMA_SQUELLETON;
...
describe('SQUELLETON integration tests', () => {
    ...
    before(async () => {
        ...
        if (!SF_SCHEMA_SQUELLETON) {
            throw "Missing SF_SCHEMA_SQUELLETON env variable";
        }
    ...
    it ('Returns the proper version', async () => {
        const query = `SELECT ${SF_DATABASEID}.${SF_SCHEMA_SQUELLETON}.VERSION() versioncol;`;
        let statement, rows;
