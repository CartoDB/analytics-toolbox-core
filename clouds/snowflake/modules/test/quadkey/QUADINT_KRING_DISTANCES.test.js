...
test('QUADINT_KRING_DISTANCES should work', async () => {
    const query = `
        SELECT QUADINT_KRING_DISTANCES(GET(VALUE,'origin'), GET(VALUE,'size')) as kring_distances
...
test('QUADINT_KRING_DISTANCES should fail if any invalid argument', async () => {
    let query = 'SELECT QUADINT_KRING_DISTANCES(NULL, NULL)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT QUADINT_KRING_DISTANCES(-1, 1)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT QUADINT_KRING_DISTANCES(162, -1)';
