...
test('Works as expected with invalid input', async () => {
    const query = `
        ...
        SELECT
            id,
            H3_DISTANCE(hid1, hid2) as distance
        FROM ids
        ORDER BY id ASC
    `;
...
test('Works as expected with valid input', async () => {
    const query = `
        ...
        (
            ...
            FROM
                distances,
                lateral FLATTEN(input =>H3_HEXRING('8928308280fffff', distance)) hid2
        )
        SELECT H3_DISTANCE(hid1, hid2) as calculated_distance, *
        FROM ids
        WHERE H3_DISTANCE(hid1, hid2) != distance;
    `;
