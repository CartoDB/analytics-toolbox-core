...
-- SQL version of indexLib.indexToTile(index) JS
...
AS $$
    CASE index
        ...
        ELSE (
            ...
            __interleaved AS (
                SELECT
                    BITAND(BITSHIFTRIGHT(index::INT, 59), 7) AS mode,
                    BITAND(BITSHIFTRIGHT(index::INT, 57), 3) AS mode_dep,
                    BITAND(BITSHIFTRIGHT(index::INT, 52), _0X1F) AS z,
                    BITSHIFTLEFT(BITAND(index::INT, _0x000FFFFFFFFFFFFF), 12) AS q
                FROM __hexConstants
            ),
            __deinterleaved1 AS (
                SELECT 
