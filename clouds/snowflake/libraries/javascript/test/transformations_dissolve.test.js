...
test('dissolve1', () => {
    ...
    const output = dissolveLib.cartoDissolve(geojson);
...
test('dissolve5', () => {
    const geojson = {"type":"GeometryCollection","properties":{},"geometries":[{"coordinates":[1,0],"type":"Point"},{"coordinates":[[[-4,1],[-4,-3],[-0,-3],[-0,1],[-4,1]]],"type":"Polygon"},{"coordinates":[[[3,-5],[3,-1],[-2,-1],[-2,-5],[3,-5]]],"type":"Polygon"}]};
    const output = dissolveLib.cartoDissolve(geojson);
    expect(output).toEqual({"type":"Feature","properties":{},"geometry":{"coordinates":[[[[-4,-3],[-2,-3],[-2,-5],[3,-5],[3,-1],[-0,-1],[-0,1],[-4,1],[-4,-3]]]],"type":"MultiPolygon"}
    });
});
