...
trait TestTables {
  self: HiveTestEnvironment =>
  // core/src/test/resources/polygons.csv when JVM is not forked
  def uriCSV: String = new File(getClass.getClassLoader.getResource("polygons.csv").getPath).toURI.toString

  def uriParquet: String = new File(getClass.getClassLoader.getResource("polygons.snappy.parquet").getPath).toURI.toString
