...
//  import HiveTestEnvironment._
  ...
  def loadSQL(path: String): List[String] = {
    Using(Source
      .fromFile(new File(getClass.getClassLoader.getResource(path).getPath))) { reader =>
      reader.mkString.split(";").toList.map(_.trim).filter(_.nonEmpty)
    }.get
  }

  def spatialFunctions: List[String] = loadSQL("sql/createUDFs.sql")
