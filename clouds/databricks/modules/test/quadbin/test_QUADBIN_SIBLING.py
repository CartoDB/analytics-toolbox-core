...
def test_quadbin_sibling_up():
    result = run_query(f"SELECT @@DB_SCHEMA@@.QUADBIN_SIBLING({QUADBIN_INDEX}, 'up')")

    assert result[0][0] == 5208061125333090303


def test_quadbin_sibling_down():
    result = run_query(f"SELECT @@DB_SCHEMA@@.QUADBIN_SIBLING({QUADBIN_INDEX}, 'down')")

    assert result[0][0] == 5209609237704998911


def test_quadbin_sibling_left():
    result = run_query(f"SELECT @@DB_SCHEMA@@.QUADBIN_SIBLING({QUADBIN_INDEX}, 'left')")
...
def test_quadbin_sibling_right():
    result = run_query(
        f"SELECT @@DB_SCHEMA@@.QUADBIN_SIBLING({QUADBIN_INDEX}, 'right')"
    )
...
def test_quadbin_sibling_none():
    result = run_query(f"SELECT @@DB_SCHEMA@@.QUADBIN_SIBLING({QUADBIN_LEVEL_0}, 'up')")
