"""Per-function timing benchmarks for Postgres modules. Sibling of test_utils."""
import atexit
import json
import os
import sys
...
def _get_bench_conn():
    ...
    if _BENCH_CONN is None:
        ...
        # Autocommit needed for running procedures since they must be
...
def _run_query_timed(sql):
    ...
    sql = sql.replace('@@PG_SCHEMA@@', os.environ.get('PG_SCHEMA', ''))
...
def _drop_bench_table(fqn):
    ...
    fqn = fqn.replace('@@PG_SCHEMA@@', os.environ.get('PG_SCHEMA', ''))
