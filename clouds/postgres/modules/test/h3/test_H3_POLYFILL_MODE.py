...
def test_h3_polyfill_mode_wrong_input():
...
def test_h3_polyfill_mode_center_polygons():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
    ...
    assert result[4][0] == 33
    assert result[5][0] == 182
    assert result[6][0] == 6


def test_h3_polyfill_mode_intersects_polygons():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
...
def test_h3_polyfill_mode_contains_polygons():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
    ...
    assert len(result) == 7
    assert result[0][0] == 1175
    assert result[1][0] == 8
    assert result[2][0] == 5
    assert result[3][0] == 13
    assert result[4][0] == 13
    assert result[5][0] == 157
    assert result[6][0] == 1


def test_h3_polyfill_mode_center_other_geometries():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
    ...
    assert result[3][0] is None
    assert result[4][0] is None
    assert result[5][0] == 16100


def test_h3_polyfill_mode_intersects_other_geometries():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
...
def test_h3_polyfill_mode_contains_other_geometries():
    """Computes polyfill for h3."""
    result = run_query(
        """
            WITH inputs AS
    ...
    assert result[3][0] is None
    assert result[4][0] is None
    assert result[5][0] == 15785


def test_h3_polyfill_mode_center_points():
    """Returns the expected values."""
    # to reproduce the same test as old implementation would need to
    # test with mode='center' the reason is that if using H3_POLYFILL
    # (e.g. mode='intersects')  also the boundary H3 are get.
...
def test_h3_polyfill_mode_intersects_points():
    """Returns the expected values."""
    # to reproduce the same test as old implementation would need to
    # test with mode='center' the reason is that if using H3_POLYFILL
    # (e.g. mode='intersects')  also the boundary H3 are get.
...
def test_h3_polyfill_mode_contains_points():
    """Returns the expected values."""
    # to reproduce the same test as old implementation would need to
    # test with mode='center' the reason is that if using H3_POLYFILL
    # (e.g. mode='intersects')  also the boundary H3 are get.
