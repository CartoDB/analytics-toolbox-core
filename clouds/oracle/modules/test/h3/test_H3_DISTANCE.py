...
def test_h3_distance_valid_inputs():
    """For each ring distance 0-4 around an origin, every cell in the
    ring must have H3_DISTANCE(origin, cell) = ring_index. Single set
    query, expect zero mismatches."""
    origin = '8928308280fffff'
    mismatches = run_query(
        f"""
        WITH distances AS (
            SELECT LEVEL - 1 AS d FROM DUAL CONNECT BY LEVEL <= 5
        )
        SELECT d, t.COLUMN_VALUE AS h3,
               @@ORA_SCHEMA@@.H3_DISTANCE('{origin}', t.COLUMN_VALUE) AS calc
        FROM distances,
             TABLE(@@ORA_SCHEMA@@.H3_HEXRING('{origin}', d)) t
        WHERE @@ORA_SCHEMA@@.H3_DISTANCE('{origin}', t.COLUMN_VALUE) != d
        """
    )
    assert (
        mismatches == 'No results returned' or mismatches == []
    ), f'Distance mismatches found: {mismatches}'
