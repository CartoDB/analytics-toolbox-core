...
def test_h3_fromlonglat_basic():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_FROMLONGLAT(-122.0553238, 37.3615593, 5)"
        " FROM DUAL"
    )
    assert len(result) == 1
...
def test_h3_fromlonglat_second_location():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_FROMLONGLAT(-164.991559, 30.943387, 5)"
        " FROM DUAL"
    )
    assert len(result) == 1
...
def test_h3_fromlonglat_null_longitude():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_FROMLONGLAT(NULL, 37.3615593, 5)"
        " FROM DUAL"
    )
    assert len(result) == 1
...
def test_h3_fromlonglat_null_latitude():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_FROMLONGLAT(-122.0553238, NULL, 5)"
        " FROM DUAL"
    )
    assert len(result) == 1
    assert result[0][0] is None
...
def test_h3_fromlonglat_world_wrapping_longitude():
    """Returns same H3 index when longitude is offset by 360 degrees."""
    result = run_query(
...
def test_h3_fromlonglat_world_wrapping_latitude():
    """Returns same H3 index when latitude is offset by 360 degrees."""
    result = run_query(
