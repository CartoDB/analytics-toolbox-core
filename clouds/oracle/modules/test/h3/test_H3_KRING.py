...
def test_h3_kring_distance_0():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_KRING('8928308280fffff', 0) FROM DUAL"
    )
    assert len(result) == 1
...
def test_h3_kring_distance_1():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_KRING('8928308280fffff', 1) FROM DUAL"
    )
    assert len(result) == 1
    cells = json.loads(result[0][0])
    expected = [
...
def test_h3_kring_distance_2():
    ...
    result = run_query(
        "SELECT @@ORA_SCHEMA@@.H3_KRING('8928308280fffff', 2) FROM DUAL"
    )
    assert len(result) == 1
    cells = json.loads(result[0][0])
    expected = [
...
def test_h3_kring_null_origin():
    ...
    with pytest.raises(Exception) as excinfo:
        run_query(
            'SELECT @@ORA_SCHEMA@@.H3_KRING(NULL, 1) FROM DUAL'
        )
...
def test_h3_kring_invalid_origin():
    ...
    with pytest.raises(Exception) as excinfo:
        run_query(
            "SELECT @@ORA_SCHEMA@@.H3_KRING('abc', 1) FROM DUAL"
        )
    assert 'Invalid input origin' in str(excinfo.value)
...
def test_h3_kring_null_size():
    ...
    with pytest.raises(Exception) as excinfo:
        run_query(
            "SELECT @@ORA_SCHEMA@@.H3_KRING('8928308280fffff', NULL)"
            " FROM DUAL"
        )
...
def test_h3_kring_negative_size():
    ...
    with pytest.raises(Exception) as excinfo:
        run_query(
            "SELECT @@ORA_SCHEMA@@.H3_KRING('8928308280fffff', -1)"
            " FROM DUAL"
        )
    assert 'Invalid input size' in str(excinfo.value)
