...
def test_s2_fromgeogpoint_success():
    results = run_query(
        """WITH resContext AS(
        ...
        SELECT @@RS_SCHEMA@@.S2_FROMGEOGPOINT(point, res) as ids
            FROM resContext;"""
    )

    with open(f'{here}/fixtures/s2_fromgeogpoint_out.txt', 'r') as fixture_file:
