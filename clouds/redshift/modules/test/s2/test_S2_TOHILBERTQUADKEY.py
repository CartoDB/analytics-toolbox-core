...
def test_s2_tohilbertquadkey_success():
    results = run_query(
        """WITH context AS(
        ...
        SELECT @@RS_SCHEMA@@.S2_TOHILBERTQUADKEY(id) AS hilbert_quadkey
        FROM context;"""
    )

    with open(f'{here}/fixtures/s2_tohilbertquadkey_out.txt', 'r') as fixture_file:
...
def test_s2_tohilbertquadkey_null_failure():
    with pytest.raises(redshift_connector.error.ProgrammingError) as excinfo:
        run_query('SELECT @@RS_SCHEMA@@.S2_TOHILBERTQUADKEY(NULL)')
