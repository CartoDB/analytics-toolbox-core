...
def test_delaunay_poly_success():
    ...
    results = run_query(
        f"""SELECT @@RS_SCHEMA@@.ST_DELAUNAYPOLYGONS(
            ST_GeomFromText('{lines[0].rstrip()}')),
        @@RS_SCHEMA@@.ST_DELAUNAYPOLYGONS(
            ST_GeomFromText('{lines[1].rstrip()}'))"""
    )

    with open(f'{here}/fixtures/st_delaunay_poly_out.txt', 'r') as fixture_file:
        lines = fixture_file.readlines()
...
def test_delaunay_poly_none():
    results = run_query(
        """SELECT @@RS_SCHEMA@@.ST_DELAUNAYPOLYGONS(
