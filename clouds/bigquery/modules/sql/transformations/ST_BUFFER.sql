...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.__BUFFER`(geojson STRING, radius FLOAT64, units STRING, steps INT64)
RETURNS STRING
DETERMINISTIC
LANGUAGE js
OPTIONS (
...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.ST_BUFFER`(geog GEOGRAPHY, radius FLOAT64, units STRING, steps INT64)
RETURNS GEOGRAPHY
AS (
    ST_GEOGFROMGEOJSON(
        `@@BQ_DATASET@@.__BUFFER`(
