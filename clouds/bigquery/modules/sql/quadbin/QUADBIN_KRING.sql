...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.__QUADBIN_ZXY_KRING`(origin STRUCT<z INT64, x INT64, y INT64>, size INT64)
AS ((
    SELECT ARRAY_AGG(
        DISTINCT (
...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.QUADBIN_KRING`(origin INT64, size INT64)
AS (
