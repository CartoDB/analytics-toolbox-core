...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.__QUADBIN_ZXY_KRING_DISTANCES`(origin STRUCT<z INT64, x INT64, y INT64>, size INT64)
AS ((
    WITH
    t AS (
        SELECT
...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.QUADBIN_KRING_DISTANCES`(origin INT64, size INT64)
AS (
