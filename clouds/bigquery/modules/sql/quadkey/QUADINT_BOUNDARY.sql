...
CREATE OR REPLACE FUNCTION `@@BQ_DATASET@@.QUADINT_BOUNDARY`(quadint INT64)
RETURNS GEOGRAPHY
AS (
    CASE
    ...
    -- Deal with level 1. Prevent error from antipodal vertices.
      ...
      ELSE COALESCE(
            ST_MAKEPOLYGON(
                ST_MAKELINE([
                    ST_GEOGPOINT(
                        `@@BQ_DATASET@@.__QUADINT_BBOX_E`(quadint),
                        `@@BQ_DATASET@@.__QUADINT_BBOX_N`(quadint)),
                    ST_GEOGPOINT(
                        `@@BQ_DATASET@@.__QUADINT_BBOX_E`(quadint),
                        `@@BQ_DATASET@@.__QUADINT_BBOX_S`(quadint)),
                    ST_GEOGPOINT(
                        `@@BQ_DATASET@@.__QUADINT_BBOX_W`(quadint),
                        `@@BQ_DATASET@@.__QUADINT_BBOX_S`(quadint)),
                    ST_GEOGPOINT(
                        `@@BQ_DATASET@@.__QUADINT_BBOX_W`(quadint),
                        `@@BQ_DATASET@@.__QUADINT_BBOX_N`(quadint)),
