...
test('QUADINT_POLYFILL should work', async () => {
    ...
    const query = `
        SELECT
            \`@@BQ_DATASET@@.QUADINT_POLYFILL\`(ST_GEOGFROMGEOJSON('${featureJSON}'), 10) as polyfill10,
            \`@@BQ_DATASET@@.QUADINT_POLYFILL\`(ST_GEOGFROMGEOJSON('${featureJSON}'), 14) as polyfill14`;
...
test('QUADINT_POLYFILL should work with GEOMETRYCOLLECTION', async () => {
    ...
    const query = `SELECT \`@@BQ_DATASET@@.QUADINT_POLYFILL\`(ST_GEOGFROMGEOJSON('${featureJSON}'), 22) as polyfill22`;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
...
test('QUADINT_POLYFILL should fail if any NULL argument', async () => {
    let query = 'SELECT `@@BQ_DATASET@@.QUADINT_POLYFILL`(NULL, 10);';
    ...
    query = `SELECT \`@@BQ_DATASET@@.QUADINT_POLYFILL\`(ST_GEOGFROMGEOJSON('${featureJSON}'), NULL)`;
    await expect(runQuery(query)).rejects.toThrow();
});