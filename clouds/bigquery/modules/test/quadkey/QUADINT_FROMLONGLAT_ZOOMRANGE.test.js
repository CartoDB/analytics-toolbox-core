...
test('QUADINT_FROMLONGLAT_ZOOMRANGE should work', async () => {
    const query = `
        SELECT
            \`@@BQ_DATASET@@.QUADINT_FROMLONGLAT_ZOOMRANGE\`(-45,30,2,10,2,5) AS agg1,
            \`@@BQ_DATASET@@.QUADINT_FROMLONGLAT_ZOOMRANGE\`(150,-30,16,18,1,5) AS agg2,
            \`@@BQ_DATASET@@.QUADINT_FROMLONGLAT_ZOOMRANGE\`(170,-80,20,24,1,4) AS agg3`;
...
test('QUADINT_FROMLONGLAT_ZOOMRANGE should return [] with NULL latitud/longitud', async () => {
    const query = `
        SELECT
            \`@@BQ_DATASET@@.QUADINT_FROMLONGLAT_ZOOMRANGE\`(NULL,10,10,12,1,5) AS agg1,
            \`@@BQ_DATASET@@.QUADINT_FROMLONGLAT_ZOOMRANGE\`(10,NULL,10,12,1,5) AS agg2`;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(rows[0].agg1).toEqual([]);
    expect(rows[0].agg2).toEqual([]);
