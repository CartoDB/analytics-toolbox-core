...
test('QUADINT_KRING should work', async () => {
    const query = `
        WITH kring_data AS
        ( SELECT row,
            \`@@BQ_DATASET@@.QUADINT_KRING\`(origin, size) as kring_elem,
        FROM UNNEST([
...
test('QUADINT_KRING should fail if any invalid argument', async () => {
    let query = 'SELECT `@@BQ_DATASET@@.QUADINT_KRING`(NULL, 1);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.QUADINT_KRING`(-1, 1);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.QUADINT_KRING`(162, NULL);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input size/);

    query = 'SELECT `@@BQ_DATASET@@.QUADINT_KRING`(162, -1);';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input size/);
});