...
test('ST_TILEENVELOPE should work', async () => {
    const query = `
        SELECT \`@@BQ_DATASET@@.ST_TILEENVELOPE\`(10, 384, 368) as geog1,
               \`@@BQ_DATASET@@.ST_TILEENVELOPE\`(18, 98304, 94299) as geog2,
               \`@@BQ_DATASET@@.ST_TILEENVELOPE\`(25, 12582912, 12070369) as geog3
    `;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
    expect(rows[0].geog1.value).toEqual('POLYGON((-45 45.089035564831, -45 44.840290651398, -44.6484375 44.840290651398, -44.6484375 45.089035564831, -45 45.089035564831))');
