...
test('H3_HEXRING should work', async () => {
    const query = `
        SELECT \`@@BQ_DATASET@@.H3_HEXRING\`('8928308280fffff', 0) as d0,
               \`@@BQ_DATASET@@.H3_HEXRING\`('8928308280fffff', 1) as d1,
               \`@@BQ_DATASET@@.H3_HEXRING\`('8928308280fffff', 2) as d2
    `;
...
test('H3_HEXRING should fail if any invalid argument', async () => {
    let query = 'SELECT `@@BQ_DATASET@@.H3_HEXRING`(NULL, NULL)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.H3_HEXRING`("abc", 1)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.H3_HEXRING`("8928308280fffff", -1)';
