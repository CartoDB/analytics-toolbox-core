...
test('H3_TOPARENT works as expected with invalid data', async () => {
    const query = `
        ...
        SELECT
            id,
            \`@@BQ_DATASET@@.H3_TOPARENT\`(hid, 1) as parent
        FROM ids
        ORDER BY id ASC
    `;
...
/* From h3-js tests:
    ...
    */
    const query = `
        ...
        FROM ids
        WHERE
            \`@@BQ_DATASET@@.H3_FROMGEOGPOINT\`(point, resolution) != \`@@BQ_DATASET@@.H3_TOPARENT\`(\`@@BQ_DATASET@@.H3_FROMGEOGPOINT\`(point, resolution + 1), resolution) OR
            \`@@BQ_DATASET@@.H3_FROMGEOGPOINT\`(point, resolution) != \`@@BQ_DATASET@@.H3_TOPARENT\`(\`@@BQ_DATASET@@.H3_FROMGEOGPOINT\`(point, resolution + 2), resolution)
    `;
