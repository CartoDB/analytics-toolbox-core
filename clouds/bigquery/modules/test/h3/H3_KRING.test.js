...
test('H3_KRING should work', async () => {
    const query = `
        SELECT \`@@BQ_DATASET@@.H3_KRING\`('8928308280fffff', 0) as d0,
               \`@@BQ_DATASET@@.H3_KRING\`('8928308280fffff', 1) as d1,
               \`@@BQ_DATASET@@.H3_KRING\`('8928308280fffff', 2) as d2
    `;
...
test('H3_KRING should fail if any invalid argument', async () => {
    let query = 'SELECT `@@BQ_DATASET@@.H3_KRING`(NULL, NULL)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.H3_KRING`("abc", 1)';
    await expect(runQuery(query)).rejects.toThrow(/Invalid input origin/);

    query = 'SELECT `@@BQ_DATASET@@.H3_KRING`("8928308280fffff", -1)';
