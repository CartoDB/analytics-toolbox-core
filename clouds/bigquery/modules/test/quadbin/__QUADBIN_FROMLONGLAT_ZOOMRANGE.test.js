...
test('__QUADBIN_FROMLONGLAT_ZOOMRANGE should work', async () => {
    const query = 'SELECT TO_JSON_STRING(`@@BQ_DATASET@@.__QUADBIN_FROMLONGLAT_ZOOMRANGE`(40.4168, -3.7038, 3, 6, 1, 4)) AS output';
...
test('__QUADBIN_FROMLONGLAT_ZOOMRANGE should return [] with NULL latitud/longitud', async () => {
    const query = `SELECT
         \`@@BQ_DATASET@@.__QUADBIN_FROMLONGLAT_ZOOMRANGE\`(NULL, -3.7038, 3, 6, 1, 4) AS output0,
         \`@@BQ_DATASET@@.__QUADBIN_FROMLONGLAT_ZOOMRANGE\`(40.4168, NULL, 3, 6, 1, 4) AS output1`;
    const rows = await runQuery(query);
    expect(rows.length).toEqual(1);
