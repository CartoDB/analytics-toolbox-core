...
describe('ST_BEARING integration tests', () => {
    ...
    it ('ST_BEARING should return NULL if any NULL mandatory argument', async () => {
        const query = `SELECT \`${BQ_PROJECTID}\`.\`${BQ_DATASET_MEASUREMENTS}\`.ST_BEARING(NULL, ST_GEOGPOINT(-4.70325 ,41.4167)) as bearing1,
        \`${BQ_PROJECTID}\`.\`${BQ_DATASET_MEASUREMENTS}\`.ST_BEARING(ST_GEOGPOINT(-3.70325 ,40.4167), NULL) as bearing2`;
