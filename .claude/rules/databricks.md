...
DB_TOKEN=<token>               # Personal access token — wins when set
DB_CLIENT_ID=<client-id>       # Service principal, used when DB_TOKEN is unset
DB_CLIENT_SECRET=<secret>      # Service principal secret; the token is renewed per job
