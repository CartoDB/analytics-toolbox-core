...
describe('ST_DESTINATION integration tests', () => {
    ...
    it ('ST_DESTINATION should return NULL if any NULL mandatory argument', async () => {
        ...
        \`${BQ_PROJECTID}\`.\`${BQ_DATASET_TRANSFORMATIONS}\`.ST_DESTINATION(ST_GEOGPOINT(-3.70325,40.4167), 10, NULL, "miles") as destination3`;
    ...
    it ('ST_DESTINATION default values should work', async () => {
        const query = `SELECT \`${BQ_PROJECTID}\`.\`${BQ_DATASET_TRANSFORMATIONS}\`.ST_DESTINATION(ST_GEOGPOINT(-3.70325,40.4167), 10, 45, "kilometers") as defaultValue,
        \`${BQ_PROJECTID}\`.\`${BQ_DATASET_TRANSFORMATIONS}\`.ST_DESTINATION(ST_GEOGPOINT(-3.70325,40.4167), 10, 45, NULL) as nullParam1`;
