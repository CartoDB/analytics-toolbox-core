...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_TRANSFORMATIONS@@.__GREATCIRCLE`
...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_TRANSFORMATIONS@@.ST_GREATCIRCLE`
    (startPoint GEOGRAPHY, endPoint GEOGRAPHY, npoints INT64)
AS (
    ST_GEOGFROMGEOJSON(`@@BQ_PROJECTID@@`.@@BQ_DATASET_TRANSFORMATIONS@@.__GREATCIRCLE(ST_ASGEOJSON(startPoint), ST_ASGEOJSON(endPoint), npoints))
);