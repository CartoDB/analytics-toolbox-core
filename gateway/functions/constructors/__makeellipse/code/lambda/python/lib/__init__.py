...
def ellipse(center, x_semi_axis, y_semi_axis, options={}):
    """
    Create an elliptical Polygon.

    Args:
        center: GeoJSON Point geometry string for center
        x_semi_axis: Semi-major axis length
        y_semi_axis: Semi-minor axis length
        options: Dict with optional 'angle', 'steps', 'units'

    Returns:
        GeoJSON Polygon geometry string
    """
    steps = 64
    if "steps" in options:
        steps = options["steps"]
...
def get_tan_deg(deg):
    """Calculate tangent from degrees."""
    rad = (deg * pi) / 180
    return tan(rad)


__all__ = ["ellipse", "get_tan_deg"]
