"""Integration tests for QUADINT_KRING function"""
...
class TestQuadintKringIntegration:
    """Integration tests for QUADINT_KRING with Redshift"""
    ...
    def test_kring_invalid_failure(self):
        """Test that invalid inputs raise errors"""
        with pytest.raises(Exception) as excinfo:
            run_query("SELECT @@RS_SCHEMA@@.QUADINT_KRING(NULL, NULL)")
        assert "Invalid input origin" in str(excinfo.value)

        with pytest.raises(Exception) as excinfo:
            run_query("SELECT @@RS_SCHEMA@@.QUADINT_KRING(-1, 1)")
        assert "Invalid input origin" in str(excinfo.value)

        with pytest.raises(Exception) as excinfo:
            run_query("SELECT @@RS_SCHEMA@@.QUADINT_KRING(162, -1)")
