"""
CARTO Analytics Toolbox - QUADINT_KRING_DISTANCES
Lambda handler for Redshift external function

Returns all quadints within a square ring with their Chebyshev distance from the origin.
"""
...
@redshift_handler
def process_quadint_kring_distances_row(row):
    """
    Get all quadints within a k-ring with distances.
    ...
    Returns:
        JSON string representing list of dictionaries with 'index' and 'distance' keys
    """
            ...
            neighbors.append(
                {
                    ...
                    "distance": max(abs(i - size), abs(j - size)),  # Chebyshev distance
                }
            )
...
lambda_handler = process_quadint_kring_distances_row
