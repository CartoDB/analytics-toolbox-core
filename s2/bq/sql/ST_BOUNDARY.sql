...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_S2@@.__GEOJSONBOUNDARY_FROMID`
    (id INT64) 
    RETURNS STRING 
    DETERMINISTIC
    LANGUAGE js
...
CREATE OR REPLACE FUNCTION `@@BQ_PROJECTID@@.@@BQ_DATASET_S2@@.ST_BOUNDARY`
    (id INT64)
AS (
    ST_GEOGFROMGEOJSON(`@@BQ_PROJECTID@@`.@@BQ_DATASET_S2@@.__GEOJSONBOUNDARY_FROMID(id))
);
